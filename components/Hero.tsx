...
export default function Hero() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      skylineRef.current.style.transform = `translateY(${y * 0.1}px)`;
    };
  ...
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden bg-bg text-ink">
      ...
      <div
        ref={skylineRef}
        className="pointer-events-none absolute inset-x-0 bottom-0 h-[340px] text-ink/55"
      >
        <Skyline className="h-full w-full" />
      </div>

      <div className="relative mx-auto w-full max-w-6xl px-6 pt-16 pb-24 lg:px-8">
        <p className="font-sans text-[11px] font-medium tracking-[0.32em] text-accent">
          LONDON · PREMIUM PROPERTY MANAGEMENT
        </p>

        <h1
          style={{ fontFamily: "var(--font-instrument)" }}
          className="mt-6 max-w-4xl text-[4.4rem] font-normal leading-[0.92] tracking-tight text-ink sm:text-[5.2rem] lg:text-[6.3rem] xl:text-[6.7rem]"
        >
        ...
        <p className="mt-7 max-w-[34rem] font-sans text-[15px] leading-relaxed text-ink/60">
